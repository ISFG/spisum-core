...
if (runScript) {
    ...
    if (!companyhome.hasAspect("ssl:pid_counter")) {
        var props = new Array(1);
        props["ssl:pid_counter_value"] = pidCounterId;
        companyhome.addAspect("ssl:pid_counter", props);
        companyhome.save();
    }

    pidCounterId = parseInt(companyhome.properties["ssl:pid_counter_value"]);

    while (true) {
        pidCounterId++;
        companyhome.properties["ssl:pid_counter_value"] = pidCounterId;
        companyhome.save();
