...
var runScript = !document.properties["ssl:pid"]
    && (
           document.typeShort == "ssl:email"
    );

if (runScript) {
    var pidCounterId = 0;

    if (!companyhome.hasAspect("ssl:pid_mailbox_counter")) {
        var props = new Array(1);
        props["ssl:pid_mailbox_counter_value"] = pidCounterId;
        companyhome.addAspect("ssl:pid_mailbox_counter", props);
        companyhome.save();
    }

    pidCounterId = parseInt(companyhome.properties["ssl:pid_mailbox_counter_value"]);

    while (true) {
        pidCounterId++;
        companyhome.properties["ssl:pid_mailbox_counter_value"] = pidCounterId;
        companyhome.save();
