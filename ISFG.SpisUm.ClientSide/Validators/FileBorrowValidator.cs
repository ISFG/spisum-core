...
{
    public class FileBorrowValidator : AbstractValidator<FileBorrow>
    {
        #region Fields
        ...
        public FileBorrowValidator(IAlfrescoHttpClient alfrescoHttpClient,
            ...
            RuleFor(o => o)
                ...
                .MustAsync(async (context, cancellationToken) =>
                {
                    ...
                    _groupRepository = await alfrescoHttpClient.GetGroupMembers(SpisumNames.Groups.RepositoryGroup,
                        ImmutableList<Parameter>.Empty.Add(
                            new Parameter(AlfrescoNames.Headers.Where, AlfrescoNames.MemberType.Group, ParameterType.QueryString)));
                ...
                {
                    RuleFor(x => x)
                        .Must(y => { return _groupPagingNextOwner?.List?.Entries?.Any(q => q.Entry.Id == y.Body.Group) ?? false; })
                        .WithName(x => "User")
                        .WithMessage("User isn't member of the Group.");

                    RuleFor(x => x)
                        .Must(y => { return _groupRepository?.List?.Entries?.Any(x => x.Entry.Id == identityUser.RequestGroup) ?? false; })
                        .WithName(x => "User")
                    ...
                    RuleFor(x => x)
                        .Must(x => _nodeEntry?.Entry?.Path?.Name?.StartsWith(AlfrescoNames.Prefixes.Path + SpisumNames.Paths.RepositoryStored,
                    ...
                    RuleFor(x => x)
                        ...
                        .WithMessage("File is already borrowed");
                });
        }
